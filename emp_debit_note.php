...
if (isset($_POST['SAVE'])){
	...
	try
	{
		...
		$debit_amount = $dbconn->GetSingleReconrd("tbl_emp_debit_note","SUM(debit_amount)","is_current=1 AND debit_status != 3 AND emp_id",$update_id);
		$return_amount = $dbconn->GetSingleReconrd("tbl_emp_debit_note","SUM(return_amount)","is_current=1 AND debit_status != 3 AND emp_id",$update_id);
		if($debit_amount==0 || $debit_amount == ''){
		$debit_amount = $_REQUEST['debit_amount'];
		}
		if($return_amount==0 || $return_amount == ''){
		$return_amount = 0;
		}
		$balance_amount = $debit_amount - $return_amount;
		
		
	$_REQUEST['pay_finyr'] = $dbconn->GetSingleReconrd("mst_finyear","finyr","finyr_active",1);
	$_REQUEST['debit_slno'] = $dbconn->GetMaxValue('tbl_emp_debit_note','debit_slno','pay_finyr',$_REQUEST['pay_finyr'])+1;
	...
	else
	{
		...
		$stmt2 = null;
		$stmt2 = $conn->prepare("INSERT INTO tbl_emp_debit_note (emp_id,debit_slno, debit_date, debit_amount, balance_amount, pay_finyr,ledger_id,pay_type,pay_cardno,pay_refno,pay_chq_no,pay_chq_dt,chq_passed,debit_remarks, is_current, debit_status, update_by, update_dtm) VALUES 
											(:emp_id,:debit_slno, :debit_date, :debit_amount, :balance_amount,:pay_finyr,:ledger_id,:pay_type,:pay_cardno,:pay_refno,:pay_chq_no,:pay_chq_dt,:chq_passed, :debit_remarks, :is_current, :debit_status, :update_by, :update_dtm)");		
		$data2 = array(
			':emp_id' => $update_id,
			':debit_slno' => $_REQUEST['debit_slno'],
			':debit_date' => date('Y-m-d',strtotime($_REQUEST['debit_date'])),
			':debit_amount' => $_REQUEST['debit_amount'],
			':balance_amount' => $_REQUEST['debit_amount'],
			...
			':debit_remarks' => $_REQUEST['debit_remarks'],
			':is_current' => 1,
			':debit_status' => 1,
		...
		$acc_main_entry = $conn->prepare("INSERT INTO tbl_accounts (acc_date, emp_id, voucher_type, record_type, debit_id, acc_tran_value, dr_ledger_id, cr_ledger_id) VALUES (:acc_date, :emp_id, :voucher_type, :record_type, :debit_id, :acc_tran_value, :dr_ledger_id, :cr_ledger_id)");		
		
		$acc_main_data = array(				
			':acc_date' => date('Y-m-d'),		
			':emp_id' => $update_id,
			':voucher_type' => "Employee Debit Note",
			':record_type' => "M",
			':debit_id' => $last_id,
			':acc_tran_value' => $_REQUEST['debit_amount'],
	...
	header("location:employee_Debit_note.php");	
	die();
}
...
<head>
    ...
    <title>
        <?php echo PAGE_TITLE; ?>- Employee Debit
    </title>
                ...
                <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
                    <div class="d-flex">
                        <div class="breadcrumb">
                            ...
                            <span class="breadcrumb-item active">Employee Debit</span>
                        </div>
                            ...
                            <fieldset>
                                <div class="card">
                                    <div class="card-header bg-pgheader text-white header-elements-inline">
                                        <h6 class="card-title">Employee Debit</h6>
                                        <div class="header-elements">
                                            <div class="list-icons">
                                                <a class="list-icons-item" href="employee_debit_note.php" title="Debit Note List"><i class="icon-arrow-left52 mr-2"></i></a>
                                        ...
                                        <div class="form-group col-lg-12 p-2">
                                            <div class="row">
                                                <div class=" col-lg-6">
                                                    <div class="card">
                                                        <div class="card-header bg-info text-white header-elements-inline">
                                                            <h6 class="card-title">Debit Details</h6>
                                                        </div>
                                                        <div class="card-body">
                                                            <div class="col-lg-12">
                                                                <div class="form-group row  pt-2">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <input type="date" name="debit_date" id="debit_date" class="form-control "  max="<?php echo date('Y-m-d'); ?>" value="<?php echo date('Y-m-d');?>" placeholder="" >
                                                                    </div>
                                                                </div>
                                                                <div class="form-group row  pt-2">
                                                                    <label class="col-lg-5 col-form-label"><b>Debit <span class="text-mandatory">*</span></b></label>
                                                                    <div class="col-lg-7">
                                                                        <input class=" form-control" type="text" name="debit_amount" id="debit_amount" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="10" value="<?php echo $obj->debit_amount; ?>" />
                                                                    </div>
                                                                </div>
                                                                <div class="form-group row  pt-2">
                                                                ...
                                                                <div class="form-group row  pt-2" id="pay_chq_dt_div">
                                                                    ...
                                                                    <!-- <input class="col-lg-6 form-control" type="text" name="" id="" value=""/> -->
                                                                    <div class="col-lg-7">
                                                                        <input type="date" name="pay_chq_dt" id="pay_chq_dt" class="form-control " max="<?php echo date('Y-m-d'); ?>" value="<?php echo date('Y-m-d');?>">
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div class="form-group row  pt-2" id="pay_chq_no_div">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <input class=" form-control" type="text" name="pay_chq_no" id="pay_chq_no" placeholder="Cheque No." value="<?php echo $obj->debit_amount; ?>" />
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div class="form-group row  pt-2" id="pay_cardno_div">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <input class=" form-control" type="text" name="pay_cardno" id="pay_cardno" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="15" placeholder="Card No." value="<?php echo $obj->debit_amount; ?>" />
                                                                    </div>
                                                                </div>
                                                                <div class="form-group row  pt-2" id="pay_refno_div">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <input class=" form-control" type="text" name="pay_refno" id="pay_refno" placeholder="Ref No." value="<?php echo $obj->debit_amount; ?>" />
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div class="form-group row  pt-2">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <textarea class="form-control" name="debit_remarks" id="debit_remarks" value=""></textarea>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                <?php
                                                    echo '<div class="card">
                                                        <div class="card-header bg-info text-white header-elements-inline">
                                                            <h6 class="card-title">Debit Update History</h6>
                                                        </div>
                                                        <div class="card-body">
                                                            <div class="col-lg-12">
                                                                <div class="row">';
                                                                ...
                                                                $SQL1 = "SELECT DISTINCT * FROM tbl_emp_debit_note WHERE emp_id = ".$emp_id." AND debit_status != 3 ORDER BY debit_date ASC";
                                                                     ...
                                                                     if ($result1->rowCount() > 0)
                                                                        {
                                                                            ...
                                                                            while ($ut = $result1->fetch()){
                                                                                ...
                                                                                echo '  <div class="col-lg-2" style="  text-align:center; padding-top:5px;">
                                                                                        ...
                                                                                        <div class="col-lg-10">
                                                                                        Debit <strong>Rs.'.$ut->debit_amount.'</strong> <span class="add-on" id="history"></span> is given by '.$dbconn->GetSingleReconrd("tbl_user","usr_name","usr_id",$ut->update_by).' on '.date("d-M-Y", strtotime($ut->update_dtm)).'';
                                                                                        if($ut->debit_remarks!=''){
                                                                                        echo '<br>Note : '.$ut->debit_remarks.'';
                                                                                        }
...
function fnValidate(){	
 ...
 if(isNull(document.thisForm.debit_date,"debit Date..!")){ return false; }

if(isNull(document.thisForm.debit_amount,"debit Amount..!")){ return false; }
...
document.thisForm.submit();
    ...
    // $('#debit_date').datepicker('setDate', 'today');
