...
if(isset($_REQUEST['id']) )
{
	...
	$html_output .= ' 
					...
					// 			// Add PHP code to fetch options from the database
					// 			$branchOptions = $dbconn->fnFillComboFromTable_Where("branch_id", "branch_name", "mst_branch", "branch_id", "WHERE branch_status = 1");
					// 			$html_output .= $branchOptions;
					// 			$html_output .= '
					// 			</select>
					// 			<script>
					// 				document.querySelector(\'#branch_id\').value = "' . $obj1->branch_id . '";
					// 			</script>
					// 			</div>';
			...
			$item_id = $_REQUEST['item_id'];
				...
				if ($result_branch->rowCount() > 0) {
					...
					if ($auto_id != "")
		            {
						...
						".$branch_data->branch_old_item_curr_stock." as ho_old_item_curr_stock, ".$branch_data->branch_old_msq." as old_item_min_qty, ".$branch_data->branch_old_moq." as old_item_order_min_qty, created_dtm FROM tbl_itemprice_history WHERE item_id = ".$item_id ." AND branch_id = ".$user_branch." Order by auto_id desc limit 10");	
				    }
						...
						".$branch_data->branch_item_cost_price." as item_cost_price, ".$branch_data->branch_stock_field." as item_curr_stock, ".$branch_data->branch_item_selling_price." as item_selling_price, ".$branch_data->branch_item_msq." as item_min_qty, 
					...
					if ($result->rowCount()>0)
					{
						while($obj=$result->fetch())
							...
							if($res->item_curr_stock != $obj->ho_old_item_curr_stock)
							{
								$curr_stock = 'C.Current Stock: '.$obj->ho_old_item_curr_stock.'<br>N.Current Stock: <span style="color:red"><b>'.$res->item_curr_stock.'</b></span>';
							}
							else{
								$curr_stock = 'C.Current Stock: '.$obj->ho_old_item_curr_stock.'<br>N.Current Stock: '.$res->item_curr_stock.'</span>';
							}
