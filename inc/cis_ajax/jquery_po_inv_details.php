...
if(isset($_POST['item_id']) && $_POST['mode']=='save')
{
	...
	if($item_dets->rowCount() > 0 ){
		...
		$row .= '<tr id="'.$itm->item_id.'" >
				...
				<td class="text-right">' . $_POST['inv_dis']  . '
					<input type="hidden" class="temp_discount_per" name="temp_discount_per[]" value="' . $_POST['inv_dis'] . '" />
					<input type="hidden" class="temp_discount_val" name="temp_discount_val[]" value="' . $_POST['inv_dis_val'] . '">
				</td>
