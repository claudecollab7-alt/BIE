...
// ini_set('display_errors', '1');ini_set('display_startup_errors', '1');error_reporting(E_ALL);
	...
	if(isset($_POST['supp_id']) && $_POST['mode'] == 'supp_item')
	{
		...
		$stmt = null;
		$stmt = $conn->prepare("SELECT * FROM tbl_item_details WHERE supp_id LIKE '%".$supp_id."%' ");
		$stmt->execute();
		$string = "";
		$string .= "" . "~" . "-- Select Item --" . "#";
		$count = $stmt->rowCount();
    ...
    if(isset($_POST['item_id']) && $_POST['mode'] == 'item_val')
	{
		...
		if($count > 0)
		{
			while($row = $stmt->fetch(PDO::FETCH_ASSOC))
			{
                ...
                $branch_item_maq = $dbconn->GetSingleReconrd("mst_branch","branch_item_maq","branch_id",$_SESSION['_user_branch']);
                ...
                $item_order_min_qty = $dbconn->GetSingleReconrd("tbl_item_stock", "$branch_item_maq", "item_id", $item_id);
