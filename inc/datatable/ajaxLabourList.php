...
$search_labour_dets = $_POST['search_labour_dets'] ? $_POST['search_labour_dets'] : '';
...
if($search_labour_dets!=''){
    $search .=" AND (a.emp_name like '%" . $search_labour_dets . "%' OR a.emp_code like '%" . $search_labour_dets . "%' OR a.emp_mobile like '%" . $search_labour_dets . "%' OR b.department_name like '%" . $search_labour_dets . "%' OR c.designation_name like '%" . $search_labour_dets . "%' )";
}
...
LEFT JOIN mst_designation as c ON c.designation_id = a.designation_id WHERE  a.rec_del_status= 1 AND a.emp_type  = 2 " .$search);
$records = $sel->fetch();
...
// print_r($cusQuery);
    ...
    $sel = $conn->query("SELECT count('a.emp_id,a.emp_code,a.emp_name,a.emp_mobile,b.department_name,c.designation_name') as allcount FROM mst_employee as a 
LEFT JOIN  mst_department as b ON a.department_id = b.department_id 
LEFT JOIN mst_designation as c ON c.designation_id = a.designation_id WHERE  a.rec_del_status= 1 AND a.emp_type  = 2 AND a.branch_id = ".$_SESSION['_user_branch']. "" .$search);
$records = $sel->fetch();
$totalRecords = $records->allcount;
...
$cusQuery ="SELECT a.emp_id,a.emp_code,a.emp_name,a.emp_mobile,b.department_name,c.designation_name FROM mst_employee as a LEFT JOIN mst_department as b ON a.department_id = b.department_id LEFT JOIN mst_designation as c ON c.designation_id = a.designation_id WHERE  a.rec_del_status= 1  AND a.emp_type = 2 AND a.branch_id = ".$_SESSION['_user_branch']. " " .$search." order by ". $columnName. " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
