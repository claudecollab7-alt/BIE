...
if ($searchByCode != '') {
    $searchQuery .= " and (a.grn_slno like '%" . $searchByCode . "%' OR b.supp_name like '%" . $searchByCode . "%'
      OR a.grn_date like '%" . $searchByCode . "%') ";
}
...
{
    $sel = $conn->query("SELECT count(*) as allcount FROM tbl_grn as a LEFT JOIN mst_supplier_new b ON a.supp_id=b.supp_id 
    WHERE a.grn_status = 2 " . $searchQuery);
    $records = $sel->fetch();
    ...
    $cusQuery = "SELECT *  from tbl_grn as a LEFT JOIN mst_supplier_new b ON a.supp_id=b.supp_id
     where a.grn_status = 2 " . $searchQuery . " GROUP BY a.grn_id order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
}
else
{
    $sel = $conn->query("SELECT count(*) as allcount FROM tbl_grn as a LEFT JOIN mst_supplier_new b ON a.supp_id=b.supp_id
    WHERE a.grn_status = 2 AND a.branch_id=".$_SESSION['_user_branch']);
    $records = $sel->fetch();
    $totalRecords = $records->allcount;

    $sel = $conn->query("SELECT count(*) as allcount FROM tbl_grn as a LEFT JOIN mst_supplier_new b ON a.supp_id=b.supp_id
    WHERE a.grn_status = 2 AND a.branch_id=".$_SESSION['_user_branch'] . $searchQuery);
    $records = $sel->fetch();
    $totalRecordwithFilter = $records->allcount;

    $cusQuery = "SELECT * from tbl_grn as a LEFT JOIN mst_supplier_new b ON a.supp_id=b.supp_id
     where a.grn_status = 2 AND a.branch_id=".$_SESSION['_user_branch']." " . $searchQuery . " GROUP BY a.grn_id order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;   
}
...
while ($obj = $cusRecords->fetch()) {
    ...
    $grn_due_date =  date("Y-m-d", strtotime($obj->party_bill_date. '+'.$supp_credit_days.' day'));
    ...
    }else if($obj->pay_amount > 0 && $obj->grn_pay_amount === ''){
        $pay_status = "<span class='badge bg-primary'>Pending</span>";
    ...
    }else if($obj->grn_pay_amount > 0 && ($obj->grn_pay_amount != $pay_amount)){

        $pay_status = "<span class='badge bg-warning'>Partial Payment</span>";
    ...
    // $grn_pay_link = '<a href="grn_pay_receipt.php?grn_id='.$obj->grn_id.'" data-popup="tooltip" title="GRN Payment"><i class="fa fa-rupee mr-2 style="font-size:16px"></i></a>';
