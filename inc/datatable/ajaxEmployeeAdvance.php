...
$search_emp_adv = $_POST['search_emp_adv'] ? $_POST['search_emp_adv'] : '';
...
if($search_emp_adv!=''){
    $search .=" AND (emp_name like '%" . $search_emp_adv . "%' OR emp_code like '%" . $search_emp_adv . "%')";
}
if ($searchByBranch != '') {
...
while($row = $result->fetch()){
    ...
    $edit_link = '<a href="emp_advance_update.php?emp_id='.$row->emp_id. '" class="badge bg-info" title="" data-original-title="Modify Advance">Modify Advance</a>';

    $advance_amount = (float)$dbconn->GetSingleReconrd("tbl_emp_advance","SUM(advance_amount)","is_current=1 AND emp_id",$row->emp_id);
	$return_amount = (float)$dbconn->GetSingleReconrd("tbl_emp_advance","SUM(return_amount)","is_current=1 AND emp_id",$row->emp_id);
    $balance_amount = (float)$dbconn->GetSingleReconrd("tbl_emp_advance","SUM(balance_amount)","is_current=1 AND emp_id",$row->emp_id);

    $advance_amount_fix = number_format(($advance_amount),2);
    ...
    $data[] = array(
        ...
        "advance_amount" => $advance_amount_fix,
