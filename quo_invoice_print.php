...
if ($_REQUEST['inv_id'] != "") {
	...
	if ($result->rowCount() > 0) {
		...
		if ($jpo_dt != "0000-00-00" && $jpo_dt != "") {
			$jpo_date = date("d-m-Y", strtotime($jpo_dt));
		}

		$dc_no = '';
		$dc_dt = '';

		if ($dc_dt != "0000-00-00" && $dc_dt != "") {
			$dc_date = date("d-m-Y", strtotime($dc_dt));
		}
		...
		$dis_count = $dbconn->GetCount("tbl_invoice_details", "inv_discount>0 AND inv_id", $obj->inv_id);
		//$dis_count_amt = $dbconn->GetSingleReconrd("tbl_invoice_details", "inv_discount_amt", "inv_id", $obj->inv_id);
	}
	...
	if($obj->cus_branch_id > 0){
		...
		if($obj1->delivery_add2 != "")
		{
			$add2 .= ', '.$obj1->delivery_add2;
		}
							...
							<div class="card-header bg-pgheader text-white header-elements-inline">
								...
								<div class="header-elements">
									<div class="list-icons">
										...
										<a class="list-icons-item" href="javascript:PrintPartsNew(new Array('print_content1'),'<?php echo $obj->inv_refno; ?>');" id="print_page" title="Print Invoice"><i class="icon-printer2 mr-1"></i></a>
										<?php }?>
										...
										<table class="table table-xs table-bordered po_print_table">
											...
											<thead>
												...
												<tr>
                                                    ...
                                                    <td colspan="4" align="left"><?php echo $state_code; ?></td>
												...
												<tr>
													<td colspan="2" style="background-color: #D3D3D3;"><b></b></td>
                                                    <td colspan="4" align="left"></td>
													<td colspan="2" style="background-color: #D3D3D3;"><b>SO.NO. </b></td>
                                                    <td colspan="2" align="left"><?php echo $so_refno ?></td>
													<td colspan="2" style="background-color: #D3D3D3;"><b>SO DATE</b></td>
                                                    <td colspan="3" align="left"><?php echo $so_dates; ?></td>
                                                </tr>
												...
												<tr>
                                                    ...
                                                    <td colspan="5" align="left"><?php echo $state_code; ?></td>
                                                </tr>
												...
												<tr style="font-weight:bold; background-color: #D3D3D3;" class="align-center uppercase">
													...
													<td width="5%" rowspan="10">Taxable Value</td>
												</tr>
												<tr style="font-weight:bold; background-color: #D3D3D3;" class="align-center uppercase">
											...
											<tbody>
												...
												echo '<tr>
												 ...
												 $pack_sql = "SELECT * FROM tbl_invoice_pack_details WHERE inv_id='" . $_REQUEST['inv_id'] . "'";
	                                                ...
	                                                if ($pack_res->rowCount() > 0) {
	                                                    ...
	                                                    {
	                                                    	$desc = $dbconn->GetSingleReconrd("mst_quo_details", "quo_pack_decp", "quo_id", $pack->inv_pack_decp);


	                                                        if ($pack->inv_pack_percent == 1) {

	                                                            $description = '<b>' . $desc . ' (' . roundofnum($pack->inv_pack_text) . ' %)</b>';
	                                                        } elseif ($pack->inv_pack_percent == 2) {
	                                                            $description = '<b>' . $desc . '</b>';
	                                                        }

	                                                        if ($pack->inv_pack_percent == 1) {
	                                                            $inv_pack_val = $pack->inv_pack_taxable_val;
	                                                        } elseif ($pack->inv_pack_percent == 2) {
	                                                            $inv_pack_val = $pack->inv_pack_taxable_val;
	                                                        }
	                                                        ...
	                                                        if ($state_code == $branch_state_code) {
	                                                            $cgst_vat = $hsn->cgst;
	                                                            $cgst_val = (((float)$inv_pack_val * (float)$cgst_vat) / 100);
	                                                            $sgst_vat = $hsn->sgst;
	                                                            $sgst_val = (((float)$inv_pack_val * (float)$sgst_vat) / 100);
	                                                        } else {
	                                                            $igst_vat = $igst;
	                                                            $igst_val = (((float)$inv_pack_val * (float)$igst_vat) / 100);
	                                                        }
	                                                        ...
	                                                        <td></td>
	                                                                <td align ='right'>";
	                                                        echo ($inv_pack_val > 0)  ? number_format($inv_pack_val, 2) : '';
	                                                        echo "</td>";

	                                                        	echo '<td class="text-right">' . number_format($cgst_vat, 2) . '</td>
		                                                            <td class="text-right">' . number_format($cgst_val, 2) . '</td>
		                                                            <td class="text-right">' . number_format($sgst_vat, 2) . '</td>
																	<td class="text-right">' . number_format($sgst_val, 2) . '</td>
																	<td class="text-right">' . number_format($igst_vat, 2) . '</td>
	                                                        ...
	                                                        $pack_total = $pack_total + $inv_pack_val;
